...
export class GetRegistryOfficeById {
    ...
    async execute(request: GetRegistryOfficeByIdRequest): Promise<GetRegistryOfficeByIdResponse> {
        const {
            id,
        } = request

        const registry_office = await this.registryOfficesRepository.findRegistryOfficeById(id);
